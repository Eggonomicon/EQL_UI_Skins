...
function Is-TopLevelWindowLocationCoordinate {
    param([System.Xml.XmlNode]$Node)

    if ($Node.Name -ne "X" -and $Node.Name -ne "Y") {
        return $false
    }

    $location = $Node.ParentNode
    if ($null -eq $location -or $location.Name -ne "Location") {
        return $false
    }

    $screen = $location.ParentNode
    if ($null -eq $screen -or $screen.Name -ne "Screen") {
        return $false
    }

    if ($null -eq $screen.ParentNode -or $screen.ParentNode.Name -ne "XML") {
        return $false
    }

    $itemAttribute = $screen.Attributes["item"]
    if ($null -eq $itemAttribute) {
        return $false
    }

    return $itemAttribute.Value -match "(Window|Wnd)$"
}
...
foreach ($file in $xmlFiles) {
    ...
    foreach ($node in $doc.SelectNodes("//*")) {
        ...
        if (Is-TopLevelWindowLocationCoordinate -Node $node) {
            continue
        }
